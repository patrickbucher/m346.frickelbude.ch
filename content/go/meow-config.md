...
- `cmd/canary`: Ein einfacher HTTP-Server fürs lokale Testen.
- `cmd/config`: Der Konfigurationsserver, der die zu monitorenden Endpoints verwaltet.
- `cmd/probe`: Der Daemon, der die _Liveness Probe_ der Endpoints durchführt und das Ergegbnis dieser Überprüfungen loggt.
